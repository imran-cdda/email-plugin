 ...
 */
export function determineContentType(html, text) {
    if (html && text)
        return "mixed";
    if (html)
        return "html";
 ...
 */
export function webhookEventToStatus(eventType) {
    switch (eventType) {
        case "email.sent":
            return "sent";
...
export function verifyWebhookSignature(payload, headers, secret) {
    try {
        ...
        if (error instanceof Error &&
            ...
            console.warn("Svix library not found - webhook signature verification disabled");
 ...
 */
export function parseResendWebhook(payload) {
 ...
 */
export function calculateEmailStats(emailLogs) {
    const total = emailLogs.length;
    const sent = emailLogs.filter((log) => log.status === "sent" || log.status === "delivered").length;
    ...
    const complained = emailLogs.filter((log) => log.status === "complained").length;
