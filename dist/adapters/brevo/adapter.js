...
export class BrevoEmailAdapter extends baseAdapter {
     ...
     */
    async getScheduledEmailStatus(limit, startDate, endDate, days, email, event, messageId, sort) {
        const response = await this.api.getEmailEventReport(limit, undefined, startDate, endDate, days, email, event, messageId, sort);
     ...
     */
    async deleteScheduledEmail(identifier) {
    ...
    async getEmailStatistics(options = {}) {
        const response = await this.api.getTransacEmailsList(undefined, undefined, options.messageId, options.startDate, options.endDate, undefined, // sort
        options.limit);
        return response.body;
    }
    // ==========================================================================
    ...
    async addDomain(domain) {
        const response = await this.executeRequest("/senders/domains", "POST", JSON.stringify({ name: domain }));
    ...
    async deleteDomain(domain) {
        const response = await this.executeRequest("/senders/domains/" + domain, "DELETE");
        return response.data;
    }
    /**
    ...
    validateEmailData(emailData) {
        ...
        else if (!emailData.messageVersions ||
        ...
        if (!emailData.htmlContent &&
            ...
            throw new Error("Either htmlContent, textContent, or templateId is required");
        }
    }
    /**
     ...
     * @returns EmailAttachment object
     */
    static createAttachmentFromUrl(url, name) {
        return { url, name };
    }
    /**
     ...
     */
    static formatScheduledDate(date) {
