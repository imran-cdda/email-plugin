...
import { arrayToString, calculateEmailStats, determineContentType, generateEmailId, parseResendWebhook, sanitizeEmailContent, validateEmailArray, } from "./utils";
 ...
 */
const createValidationError = (status, message, code, field, additionalData) => {
...
export const email = (options) => {
    ...
    const getDefaultAdapter = () => {
        ...
        if (!adapter) {
            throw createValidationError("INTERNAL_SERVER_ERROR", `Email adapter '${defaultProvider}' not found`, EMAIL_ERROR_CODES.ADAPTER_NOT_FOUND);
        }
        return adapter;
    };
    const emailEndpoints = {
         ...
         */
        sendEmail: createAuthEndpoint("/email/send", {
            method: "POST",
            body: z
                .object({
            ...
            try {
                ...
                const emailLog = await ctx.context.adapter.create({
                    model: "emailLog",
                    data: {
                        ...emailLogData,
                        createdAt: new Date(),
                ...
                emailLog.id = emailLogId;
                // If send failed, throw error after logging
                if (!sendResult.success) {
                    throw createValidationError("INTERNAL_SERVER_ERROR", sendResult.error || "Failed to send email", EMAIL_ERROR_CODES.SEND_FAILED);
                }
                return {
                    success: true,
                    emailId: emailLog.id,
         ...
         */
        sendBulkEmails: createAuthEndpoint("/email/send-bulk", {
            method: "POST",
            body: z.object({
                emails: z.array(z
                    .object({
            ...
            try {
                ...
                for (const email of emailRequests) {
                    if (!email.from) {
                        throw createValidationError("BAD_REQUEST", "From address is required for all emails", EMAIL_ERROR_CODES.INVALID_EMAIL_ADDRESS);
                    }
                }
                // Send bulk emails
                ...
                throw createValidationError("INTERNAL_SERVER_ERROR", error instanceof Error ? error.message : "Unknown error occurred", EMAIL_ERROR_CODES.BULK_SEND_FAILED);
            }
        }),
        /**
         * ### Endpoint
         ...
         */
        getEmailLogs: createAuthEndpoint("/email/logs", {
            method: "GET",
            query: z
                .object({
            ...
            const whereConditions = [{ field: "userId", value: query.userId || "" }];
            if (query.status) {
            ...
            const emailLogs = await ctx.context.adapter.findMany({
                model: "emailLog",
         ...
         */
        getEmailStats: createAuthEndpoint("/email/stats", {
            method: "GET",
            query: z
                .object({
                userId: z.string().optional(),
            ...
            const stats = calculateEmailStats(emailLogs);
            return {
                success: true,
                stats,
                userId,
         ...
         */
        sendSystemEmail: createAuthEndpoint("/email/send-system", {
            method: "POST",
            body: z
                .object({
                to: z.array(z.object({ email: z.email(), name: z.string().optional() })),
        ...
        handleEmailWebhook: createAuthEndpoint("/email/webhook", {
            ...
            try {
                ...
                const webhookSecret = options?.webhookSecret || process.env.RESEND_WEBHOOK_SIGNING_SECRET;
                console.log("[EMAIL WEBHOOK] Webhook secret configured:", !!webhookSecret);
                ...
                if (webhookSecret) {
                    ...
                    if (headers["svix-signature"]) {
                        ...
                        const isValid = verifyWebhookSignature(body, headers, webhookSecret);
                        if (!isValid) {
                        ...
                        console.log("[EMAIL WEBHOOK] No signature header found, skipping verification");
                    }
                }
                else {
                    console.log("[EMAIL WEBHOOK] No webhook secret configured, skipping verification");
                }
                // Parse webhook payload (assuming Resend format for now)
                let payload;
                try {
                    payload = JSON.parse(body);
                    console.log("[EMAIL WEBHOOK] Parsed payload:", JSON.stringify(payload, null, 2));
                }
                catch (parseError) {
                    console.error("[EMAIL WEBHOOK] Failed to parse JSON payload:", parseError);
                    throw new APIError("BAD_REQUEST", {
                ...
                console.log("[EMAIL WEBHOOK] Looking for email log with providerId:", messageId);
                if (!messageId) {
                    console.error("[EMAIL WEBHOOK] No message ID found in webhook payload");
                    throw new APIError("BAD_REQUEST", {
                        message: "No message ID found in webhook payload",
                    });
                }
                const emailLog = await ctx.context.adapter.findOne({
                    model: "emailLog",
                    where: [{ field: "providerId", value: messageId }],
                });
                if (!emailLog) {
                    // Log warning but don't throw error - this might be a valid case
                    console.warn(`[EMAIL WEBHOOK] Email log not found for message ID: ${messageId}. This might be a webhook for an email not sent through this system.`);
                ...
                console.log("[EMAIL WEBHOOK] Update data:", JSON.stringify(updateData, null, 2));
                const updatedEmailLog = await ctx.context.adapter.update({
                    model: "emailLog",
                    where: [{ field: "id", value: emailLog.id }],
                    update: updateData,
                });
                console.log("[EMAIL WEBHOOK] Successfully updated email log:", emailLog.id);
                return {
                    success: true,
                ...
                throw createValidationError("INTERNAL_SERVER_ERROR", error instanceof Error ? error.message : "Unknown error occurred", EMAIL_ERROR_CODES.INVALID_WEBHOOK_PAYLOAD);
            }
        }),
    };
    return {
...
export class EmailService {
    ...
    static getInstance(baseUrl) {
        if (!baseUrl && !process.env.BETTER_AUTH_URL) {
            throw new Error("Base URL must be provided or BETTER_AUTH_URL environment variable must be set");
        }
        if (!EmailService.instance) {
            EmailService.instance = new EmailService(baseUrl || process.env.BETTER_AUTH_URL || "");
        }
        return EmailService.instance;
    }
    async sendSystemEmail(emailData) {
        try {
            const response = await fetch(`${this.baseUrl}/api/auth/email/send-system`, {
                method: "POST",
                headers: {
...
// Email templates
const verificationEmailTemplate = (user, verifyUrl) => `
